...
SG_ID="$(aws ec2 describe-security-groups  --filters Name=group-name,Values=B55_ALLOW_ALL_SECURITY_GROUP | jq '.SecurityGroups[].GroupId' | sed -e 's/"//g')"       # b54-allow-all security group id
